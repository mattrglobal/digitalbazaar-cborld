...
describe('cborld', () => {
  describe('encode', () => {
    ...
    it('should support encoding a JSON-LD document with undefined terms', async () => {
      const CONTEXT_URL = 'urn:foo';
      const CONTEXT = {
        '@context': {
      ...
      const promise2 = encode({
        jsonldDocument,
        documentLoader,
        allowUndefinedTerms: false
      });
      const promise3 = encode({
        jsonldDocument,
        documentLoader,
        allowUndefinedTerms: true
      });

      await expect(promise1).rejectedWith("Unknown term 'bar' was detected in the JSON-LD input");
      await expect(promise2).rejectedWith("Unknown term 'bar' was detected in the JSON-LD input");

      await expect(promise3).fulfilled;
      expect(await promise3).equalBytes('d90501a3006775726e3a666f6f1866016362617202');
    })
  });
  ...
  describe('decode', () => {
    ...
    it('should decode CBOR-LD document with undefined terms', async () => {
      ...
      const cborldBytes = _hexToUint8Array('d90501a3006775726e3a666f6f1866016362617202');
      ...
      await expect(promise1).rejectedWith("Unknown term ID 'bar' was detected in the CBOR-LD input");
      await expect(promise2).rejectedWith("Unknown term ID 'bar' was detected in the CBOR-LD input");
