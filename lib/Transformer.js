...
export class Transformer {
  ...
  async _transform({obj, transformMap, contextStack = []}) {
    ...
    for(const [termInfo, value] of termEntries) {
      ...
      const termType = this._getTermType({
        activeCtx: newActiveCtx || activeCtx,
        def,
      });
  ...
  async _updateContextStack({contextStack, contexts}) {
    ...
    for(const context of contexts) {
      // console.log('_updateContextStack context: ', context);
      let entry = contextMap.get(context);
      // console.log('_updateContextStack entry: ', entry);
      if(!entry) {
        let ctx = context;
        let contextUrl;
        ...
        // add context
        console.log('_updateContextStack contextUrl: ', contextUrl);
        console.log('_updateContextStack ctx: ', ctx);

        // Ivy: context here could be a mix of strings (context urls)
        // and objects
        if(Array.isArray(ctx)) {
          console.log('_updateContextStack isArray(ctx): ', true);
          await this._updateContextStack({contextStack, contexts: ctx});
          return;
        }

        entry = await this._addContext({
          context: ctx,
          contextUrl,
        });
      }
  ...
  // Ivy: context here is expected to be an object
  ...
  _getTermType({activeCtx, def}) {
    ...
    if(
      !(typeof prefixDef === 'object' && typeof prefixDef['@id'] === 'string')
    ) {
