...
export class Decompressor extends Transformer {
   ...
   *   (0x7FFF)).
   * @param {boolean} [options.allowUndefinedTerms] - Allow the JSON-LD document
   *   to contain terms that are not defined in the context. The original values
   *   of terms that cannot be resolved from the context map will be preserved.
   */
  constructor({documentLoader, appContextMap, allowUndefinedTerms} = {}) {
    super({documentLoader, appContextMap, allowUndefinedTerms});
  ...
  _getEntries({transformMap, termMap}) {
    ...
    for(const [key, value] of transformMap) {
      ...
      if(term === undefined && !this.allowUndefinedTerms) {
        throw new CborldError(
        ...
        if(typeof key !== 'string') {
          throw new CborldError(
            ...
            `Unknown term ID "${key}" was detected in the CBOR-LD input. ` +
              ...
              `"allowUndefinedTerms" is enabled. The JSON-LD document must ` +
              `also be encoded with "allowUndefinedTerms" enabled.`);
        }
        entries.push([{
          term: key,
          termId: key,
